...
const EMAILJS_SERVICE_ID = "service_gakf7oo";
const EMAILJS_TEMPLATE_ID = "template_m7yry77";
const EMAILJS_PUBLIC_KEY = "5UYKY19R249bxfODW";
...
// ===== Email Notification =====
function sendEmail(title, body, dueDate) {
  const templateParams = {
    title: title,
    body: body,
    dueDate: dueDate ? new Date(dueDate).toLocaleString() : "No due date",
    email: "borkar3232@gmail.com",
  };

  console.log("Calling SendEmail", templateParams);
...
// ===== Task Reminder Logic (check every 20 seconds) =====
setInterval(() => {
  const now = Date.now();
  tasks.forEach((t) => {
    if (t.due && !t.reminded) {
      ...
      if (now >= d) {
        sendEmail(t.title, t.notes, t.due); // Send the email here
        t.reminded = true; // Avoid sending the email again
        t.updatedAt = new Date().toISOString();
        saveTasks();
      }
    }
