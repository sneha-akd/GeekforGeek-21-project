...
const user = JSON.parse(localStorage.getItem('quickslot-user'));
...
function sendConfirmationEmail(booking) {
  ...
  console.log(`Booking Details: Provider: ${booking.provider}, Date: ${booking.date}, Slot: ${booking.slot}`);
  alert(`Confirmation email sent to ${user.email} (simulated)`);
}
...
function renderSlots(providerId, date) {
  ...
  slots.forEach((slot) => {
    ...
    card.innerHTML = `
      ...
      <div class="small text-secondary">${slot.disabled ? "Unavailable" : "Tap to book"
      }</div>
    `;
      ...
      card.innerHTML = `
        <div class="d-flex justify-content-between align-items-start gap-3">
          <div>
            ...
            <div class="small text-secondary">${booking.date} · ${booking.slot
        }</div>
...
function sendConfirmationEmail(booking) {
  ...
  const emailContent = `
📧 QUICK SLOT — BOOKING CONFIRMATION
-----------------------------------
To: ${user.email}
...
(This is a simulated email. No real email was sent.)
  ...
  console.log(emailContent);  // Appears in browser console for debugging

  alert(`A confirmation mail has been sent to ${user.email} (dummy simulation).`);
}
async function init() {
  readBookings();
