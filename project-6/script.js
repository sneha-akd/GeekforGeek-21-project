...
const schema = [
  {
    id: "identity",
    title: "Identity",
    icon: "fas fa-user",  // Add an icon class here
    fields: [
      {
        key: "fullName",
        label: "Full name",
  ...
  },
  {
    id: "contact",
    title: "Contact",
    icon: "fas fa-phone-alt",  // Add an icon class here
    fields: [
      {
        key: "email",
        label: "Email",
  ...
  },
  {
    id: "skills",
    title: "Skills",
    icon: "fas fa-cogs",  // Add an icon class here
    fields: [
      {
        key: "skills",
  ...
  },
  {
    id: "experience",
    title: "Experience",
    icon: "fas fa-briefcase",  // Add an icon class here
    repeatable: true,
    fields: [
  ...
  },
  {
    id: "education",
    title: "Education",
    icon: "fas fa-graduation-cap",  // Add an icon class here
    repeatable: true,
    fields: [
  ...
  minimal: {
    ...
    render: (data) => `
      <header>
        ...
        <p>${joinValues([data.email, data.phone, data.location, data.website], " · ")}</p>
      </header>
      <section>
      ...
      (item) => `
          <div class="mb-3">
            <strong>${item.title || "Role"}</strong> — ${item.company || "Company"}
      ...
      (item) => `
          <div class="mb-3">
            <strong>${item.degree || "Degree"}</strong>, ${item.school || "School"}
            <div class="text-muted">${item.period || "Timeline"}</div>
            <p>${item.details || ""}</p>
          </div>
        `
    ...
    render: (data) => `
      <header class="mb-4">
        ...
        <div class="d-flex flex-wrap gap-2 text-muted">
          ${drawBadges(cleanValues([data.email, data.phone, data.location, data.website]))}
        </div>
      </header>
      <section class="mb-4">
      ...
      (item) => `
            <div>
              ...
              <div class="text-muted small">${item.company || "Company"} · ${item.period || "Timeline"}</div>
              ${drawHighlights(item.highlights)}
            </div>
          `
    )}
      ...
      (item) => `
            <div>
              ...
              <div class="text-muted small">${item.school || "School"} · ${item.period || "Timeline"}</div>
...
function buildForm() {
  ...
  schema.forEach((section) => {
    ...
    heading.innerHTML = `${getIconForSection(section.id)} ${section.title}`;
...
function getIconForSection(sectionId) {
  switch (sectionId) {
    case 'identity':
      return '<i class="fas fa-user"></i>'; // Identity icon
    case 'contact':
      return '<i class="fas fa-phone-alt"></i>'; // Phone icon for Contact
    case 'skills':
      return '<i class="fas fa-cogs"></i>'; // Skills icon
    case 'experience':
      return '<i class="fas fa-briefcase"></i>'; // Experience icon
    case 'education':
      return '<i class="fas fa-graduation-cap"></i>'; // Education icon
    default:
      return '<i class="fas fa-info-circle"></i>'; // Default info icon
  }
}
...
function saveHtml(template, prepared) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${prepared.fullName || "resume"
    }</title><style>${EXPORT_STYLES}</style></head><body class="${template.className
